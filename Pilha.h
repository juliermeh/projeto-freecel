...
{
    int valor; //No futuro, achar um método de transformar este tipo em algo mais dinâmico
    struct ItemList *proximo;
}Item;

typedef struct PilhaStructure
{
    Item *start;
...
int adicionarNovoItemPilha(Pilha* pilha, int valor);
int adicionarItemPilha(Pilha* pilha, Item* item);
...
Item* removeTopItemPilha(Pilha* pilha);
...
Item* getPreviousItemOnPilha(Item *item, Pilha *pilha);
